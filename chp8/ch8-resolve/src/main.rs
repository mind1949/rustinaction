...
fn main() {
    let arg_dns_server = "dns-server";
    let arg_domain_name = "domain-name";
    let app = App::new("resolve")
        .about("A simple to use DNS resolver")
        .arg(
            Arg::with_name(arg_dns_server)
                .short("s")
                .default_value("1.1.1.1"),
        )
        .arg(Arg::with_name(arg_domain_name).required(true))
        .get_matches();

    let domain_name_raw = app.value_of(arg_domain_name).unwrap();
    let domain_name = Name::from_ascii(&domain_name_raw).unwrap();

    let dns_server_raw = app.value_of(arg_dns_server).unwrap();
